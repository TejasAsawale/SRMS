...
//         try {
//             if (isSignup) {
//                 // Map local state to backend model keys
//                 const studentData = {
...
//                         {/* New Fields: Gender & DOB in one row if your CSS supports it */}
...
const Dashboard = ({ onLogin }) => {
    const { theme } = useTheme(); // Access the global theme state
    const [isSignup, setIsSignup] = useState(false);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState('');
    const [success, setSuccess] = useState('');
    ...
    return (
        /* Apply the theme class here so CSS variables update */
