...
// ... (CSV/PDF Export utilities stay here) ...
...
const ResultsPage = () => {
    ...
    // fetchData logic ... (Keeping it for context)
    const fetchData = useCallback(async () => {
        try {
            setLoading(true);
            const [classRes, studRes, subRes, resData] = await Promise.all([
    ...
    // Handle Declaring Submit
    const handleDeclareSubmit = async (e) => {
        e.preventDefault();
        try {
            const payloads = Object.keys(marksInput).map(subCode => ({
                RollId: formStudent,
                SubjectCode: subCode,
                Marks: marksInput[subCode]
            }));
            await Promise.all(payloads.map(p => axios.post(`${API}/results/add`, p)));
            alert("Results saved!");
            setMarksInput({});
            ...
            {activeTab === "view" ? (
                <>
                    <div className="class-grid">
                        {classes.map(cls => (
                            <div className="class-card" key={cls._id}>
                                <button className="delete-card-btn" onClick={() => {}}>🗑️</button>
                                <div className="card-icon">📁</div>
                                <div className="class-name">{cls.ClassName}</div>
                                <div className="card-actions">
                                    <button className="btn-outline" onClick={() => {}}>PDF</button>
                                    <button className="btn-outline" onClick={() => {}}>CSV</button>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="section-card">
                        <div className="filter-bar">
                            <input className="search-input" placeholder="Search by name or roll..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
                            <select className="class-filter-select" value={selectedClass} onChange={e => setSelectedClass(e.target.value)}>
                                <option value="All Classes">All Classes</option>
                                {classes.map(c => <option key={c._id} value={c.ClassName}>{c.ClassName}</option>)}
                            </select>
                        </div>
                                ...
                                <tbody>
                                    {filteredStudents.map(s => (
                                        <tr key={s._id}>
                                            ...
                                            <td>
                                                <div className="action-btns">
                                                    <button className="btn-report" onClick={() => {}}>View Report</button>
                                                    <button className="btn-delete-row" onClick={() => {}}>Delete</button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
                ...
                <div className="section-card declare-section">
                    <form className="declare-form" onSubmit={handleDeclareSubmit}>
                        <h2>Declare Student Result</h2>
                        <div className="form-group">
                            <label>Select Class</label>
                            <select className="class-filter-select" value={formClass} onChange={e => {setFormClass(e.target.value); setFormStudent("");}} required>
                        ...
                        {formClass && (
                            <div className="form-group">
                                ...
                                <select className="class-filter-select" value={formStudent} onChange={e => setFormStudent(e.target.value)} required>
                                    <option value="">-- Choose Student --</option>
                                    {students.filter(s => s.ClassId === formClass).map(s => (
                        ...
                        {formStudent && (
                            <div className="marks-input-container">
                                {subjects.filter(sub => sub.Class === formClass).map(sub => (
                                    <div key={sub.SubjectCode} className="marks-row">
                                        <label>{sub.SubjectName}</label>
                                        <input className="search-input" type="number" required onChange={e => setMarksInput({...marksInput, [sub.SubjectCode]: e.target.value})} />
                                    </div>
                                ))}
                                <button type="submit" className="btn-primary">Save Academic Records</button>
                            </div>
                        )}
                    </form>
                </div>
