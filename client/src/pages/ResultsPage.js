...
//     // ── FIX 1: Delete all results for a class ────────────────────────────────
...
//             }
//             await Promise.all(
//                 classStudents.map(s => axios.delete(`${API}/results/deleteByRoll/${s.RollId}`))
//             );
...
//     // Get class name from a subject — checks every possible API field name
//     const getSubjectClass = (sub) => sub.Class || sub.ClassName || sub.ClassId || sub.classId || "";

//     // Get class name from a class object — checks every possible API field name
//     const getClassName = (cls) => cls.ClassName || cls.name || cls.Name || cls.ClassId || cls.classId || "";

//     // Derive active classes purely from students+subjects — bypasses any class API field mismatch.
//     // A class is active if: at least one student has that ClassId AND at least one subject targets that same ClassId.
...
//     // Build activeClasses as objects with a guaranteed ClassName field
...
//                                     {/* FIX: wired to handleDeleteClassResults */}
//                                     <button
...
//                             />
//                             {/* FIX: dropdown shows only classes with enrolled students */}
//                             <select
...
const ResultsPage = () => {
    ...
    const handleDeleteClassResults = async (className) => {
        ...
        try {
            ...
            // Get result IDs for each student, then delete by _id (backend route: DELETE /delete/:id)
    ...
    const handleDeleteSingle = async (rollId, name) => {
        if (!window.confirm(`Delete results for ${name}?`)) return;
        try {
            // Get all result entries for this student, delete each by _id
